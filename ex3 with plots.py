...
def calculate_accuracy(topic_names, articles_topics, correct_topics):
    ...
    confusion_matrix = np.zeros((num_of_cluster, num_of_cluster + 1))
    clusters_in_topic = np.zeros(num_of_cluster)
    ...
    for cluster in range(num_of_cluster):
        ...
        for i, name in enumerate(topic_names):
            ...
            confusion_matrix[cluster, i] = cur_num_of_success
        ...
        clusters_in_topic[final_topic[0]] += len(indexes)
        
    # sort the rows of confusion_matrix by the sum of each row and print a list of the new row order
    order_by_row_sum = np.argsort(np.sum(confusion_matrix, axis=1))[::-1]
    confusion_matrix = confusion_matrix[order_by_row_sum]
    confusion_matrix[:, -1] = np.sum(confusion_matrix, axis=1)
    
    # confusion matrix
    fig, ax = plt.subplots()
    ax.imshow(confusion_matrix)
    x_labels = topic_names + ['cluster size']
    ax.set_xticks(np.arange(len(x_labels)), labels=x_labels)
    ax.set_yticks(np.arange(len(order_by_row_sum)), labels=order_by_row_sum)
    ...
    for i in range(num_of_cluster):
        ax.text(len(x_labels) - 1, i, int(confusion_matrix[i, -1]), ha="center", va="center", color="w")
    ...
    # Histogram of the number of articles in each cluster
    plt.bar(topic_names, clusters_in_topic)
    plt.xlabel('Topics')
    plt.ylabel('Articles in custer')
...
def cluster_articles(articles):
    ...
    # Creates a graph for the likelihood in which the X axis is iteration number and the Y axis is the log likelihood.
    ...
    # Creates a graph for the perplexity in which the X axis is iteration number and the Y axis is the perplexity.
